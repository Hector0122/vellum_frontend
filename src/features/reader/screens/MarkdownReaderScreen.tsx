...
import { splitMarkdownBlocks, type MarkdownBlock } from '@/features/reader/utils/markdownBlocks';
...
const mdStyles = StyleSheet.create({
  ...
  heading1: { color: colors.text, fontSize: 26, fontWeight: '700', marginTop: 8 },
  heading2: { color: colors.text, fontSize: 22, fontWeight: '700', marginTop: 8 },
  heading3: { color: colors.text, fontSize: 19, fontWeight: '700', marginTop: 6 },
  code_inline: { backgroundColor: colors.elevated, color: colors.text },
  code_block: { backgroundColor: colors.elevated, color: colors.text },
  fence: { backgroundColor: colors.elevated, color: colors.text },
  link: { color: colors.accent },
});

/**
 * Markdown rendering via react-native-markdown-display inside a ScrollView.
 * Highlights and progress are block-level, not text-range — see design.md's
 * "Revised during implementation" note in
 * openspec/changes/add-multi-format-documents: the rich renderer builds a
 * tree of native views, which rules out the character-offset selection
 * trick (a plain-text TextInput overlay) without sacrificing rich display.
 */
export function MarkdownReaderScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const cached = await isDocumentCached(bookId, 'md');
      if (!cached) {
        try {
          await downloadAndCacheDocument(bookId, 'md');
        } catch (e) {
          if (__DEV__) console.error('Failed to download document:', e);
          setReaderError('Failed to download document');
          return;
        }
      } else {
        // Confirm the cached file is actually reachable before trusting it.
        const path = await getCachedDocumentPath(bookId, 'md');
        if (!path) {
      ...
      const text = await readCachedMarkdown(bookId);
      if (text === null) {
        setReaderError('Document file not found');
        return;
      }

      setBlocks(splitMarkdownBlocks(text));

      const initialLocator = parseLocator(book?.progress_locator);
      if (initialLocator && isMarkdownLocator(initialLocator)) {
        restoreTargetRef.current = initialLocator.blockIndex;
      } else {
        restoredRef.current = true; // nothing to restore, start from top
      }
  ...
  const persistProgress = useCallback(
    (blockIndex: number, scrollOffset: number) => {
      if (progressTimeoutRef.current) clearTimeout(progressTimeoutRef.current);
      progressTimeoutRef.current = setTimeout(() => {
        progressTimeoutRef.current = null;
        const percent = blocks.length > 0 ? Math.min(Math.round((blockIndex / blocks.length) * 100), 100) : 0;
        updateProgress(bookId, percent, buildMarkdownLocator(blockIndex, scrollOffset));
      }, PROGRESS_DEBOUNCE_MS);
    },
    [bookId, blocks.length, updateProgress],
  );
  ...
  const handleBlockLayout = useCallback((index: number, event: LayoutChangeEvent) => {
    blockYRef.current[index] = event.nativeEvent.layout.y;

    if (!restoredRef.current && restoreTargetRef.current === index) {
      restoredRef.current = true;
      const y = blockYRef.current[index] ?? 0;
      // Defer to let layout settle before jumping.
  ...
  const handleScroll = useCallback(
    (event: NativeSyntheticEvent<NativeScrollEvent>) => {
      ...
      const ys = blockYRef.current;
      let nearest = 0;
      for (let i = 0; i < ys.length; i++) {
        if (ys[i] !== undefined && ys[i] <= scrollY + TOP_EPSILON) nearest = i;
      }
      setCurrentBlockIndex(nearest);
      if (restoredRef.current) persistProgress(nearest, scrollY);
    },
    [persistProgress],
  );
  ...
  const currentBlockHighlight = useMemo(
    () =>
      highlights.find((h) => {
        const locator = parseLocator(h.locator);
        return locator && isMarkdownLocator(locator) && locator.blockIndex === currentBlockIndex;
      }),
    [highlights, currentBlockIndex],
  );

  const handleToggleHighlight = useCallback(() => {
    if (currentBlockHighlight) {
      deleteHighlight(bookId, currentBlockHighlight.id)
        .then(() => {
          hapticLight();
    ...
    const text = block ? block.text.slice(0, 200) : `Block ${currentBlockIndex}`;
    createHighlight(bookId, text, buildMarkdownLocator(currentBlockIndex), colors.highlightYellow)
      .then(() => {
        hapticSuccess();
        showToast('success', 'Block highlighted');
      })
      .catch((err: any) => showToast('error', 'Error', err.message));
  }, [currentBlockHighlight, blocks, bookId, currentBlockIndex, deleteHighlight, createHighlight]);

  const handleAddBookmark = useCallback(async () => {
    try {
      const block = blocks[currentBlockIndex];
      const label = block ? block.text.slice(0, 50) : `Block ${currentBlockIndex}`;
      await addBookmark(bookId, buildMarkdownLocator(currentBlockIndex), label);
      hapticSuccess();
      showToast('success', 'Bookmark added');
    } catch {
      showToast('error', 'Failed to add bookmark');
    }
  }, [addBookmark, bookId, blocks, currentBlockIndex]);

  const goToLocator = useCallback((rawLocator: string, closePanels: () => void) => {
    const locator = parseLocator(rawLocator);
    if (!locator || !isMarkdownLocator(locator)) {
      showToast('error', 'Invalid location');
      return;
    }
    const y = blockYRef.current[locator.blockIndex] ?? 0;
    scrollRef.current?.scrollTo({ y, animated: true });
    setCurrentBlockIndex(locator.blockIndex);
    closePanels();
  }, []);
  ...
  const handleShowSummary = useCallback(() => {
    const sectionIndex = blocks[currentBlockIndex]?.sectionIndex ?? 0;
    resetSummary();
    setShowOverlay(false);
    setShowSummary(true);
    fetchSummary(sectionIndex, String(sectionIndex));
  }, [blocks, currentBlockIndex, resetSummary, fetchSummary]);
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
        ...
        >
          {blocks.map((block, index) => {
            const isHighlighted =
              highlights.some((h) => {
                const locator = parseLocator(h.locator);
                return locator && isMarkdownLocator(locator) && locator.blockIndex === index;
              });
            return (
              <View
                key={index}
                onLayout={(e) => handleBlockLayout(index, e)}
                style={isHighlighted ? styles.highlightedBlock : undefined}
              >
                <Markdown style={mdStyles}>{block.text}</Markdown>
              </View>
            );
          })}
        </ScrollView>
            ...
            <View style={styles.actionRow}>
              ...
              <TouchableOpacity style={styles.actionBtn} onPress={handleToggleHighlight}>
                <Text style={styles.actionBtnText}>
                  {currentBlockHighlight ? 'Unhighlight block' : 'Highlight block'}
                </Text>
              </TouchableOpacity>
      ...
      {showHighlights && (
        ...
        >
          ...
          {highlights.length === 0 ? (
            <Text style={styles.noHighlights}>Highlight a block to see it here</Text>
          ) : (
            <FlatList
              data={highlights}
              keyExtractor={(h) => h.id}
...
const styles = StyleSheet.create({
  ...
  highlightedBlock: {
    backgroundColor: colors.highlightYellow + '33',
