...
// This screen is currently EPUB-only (ReadiumView doesn't render PDF/Markdown
// yet — see tasks 4.x/5.x in openspec/changes/add-multi-format-documents),
// so it narrows every locator to the Readium shape via parseEpubLocator.
import { parseEpubLocator as parseLocator } from '@/features/reader/utils/parseLocator';
...
// EPUB rendering via react-native-readium. Split out of ReaderScreen (which is
// now a thin format dispatcher) when PDF/Markdown support was added — see
// openspec/changes/add-multi-format-documents. Deliberately left otherwise
// unchanged (design.md: "keep the EPUB path completely unchanged").
